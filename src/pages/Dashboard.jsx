    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard title="Total Revenue" value={`$${totalRevenue}`} icon={LayoutDashboard} color="border-purple-500" />
        </div>
          ...
          <ul className="divide-y divide-gray-200">
            {data.orders.slice(0, 5).map(order => (
              <li key={order.id} className="py-3 flex justify-between items-center">
                ...
                <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${order.status === 'Shipped' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                  {order.status}
                </span>
